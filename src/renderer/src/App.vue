                  ...
                  >
                    ...
                    >
                      ...
                      <span class="ml-auto text-[9px] px-1.5 py-0.5 rounded-md bg-primary/10 text-primary font-bold">{{
                        chatMode === 'dialogue' ? '仅对话' : chatMode === 'descriptive' ? '包含描写' : '导演模式'
                      }}</span>
                    </button>
      ...
      <div class="modal-panel max-w-sm w-full p-6 space-y-5 animate-fade-in shadow-2xl rounded-2xl border border-outline-variant/10 bg-surface/85 backdrop-blur-[30px] select-none text-on-surface">
        ...
        <div class="space-y-2.5">
          ...
          <button
            ...
            :class="chatMode === 'dialogue' ? 'border-primary bg-primary/5' : 'border-outline-variant/40 hover:border-primary/40 hover:bg-surface-high'"
          >
            <div class="flex items-center space-x-3">
              <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" :class="chatMode === 'dialogue' ? 'bg-primary/15 text-primary' : 'bg-surface-high text-on-surface-variant'">
                <MessageSquareIcon class="w-4 h-4" />
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center space-x-2">
                  <span class="text-xs font-bold" :class="chatMode === 'dialogue' ? 'text-primary' : 'text-on-surface'">仅对话</span>
                  <span v-if="chatMode === 'dialogue'" class="text-[8px] px-1.5 py-0.5 rounded-md bg-primary text-white font-bold">当前</span>
                </div>
          ...
          <button
            ...
            class="w-full text-left p-4 rounded-2xl border-2 transition-all focus:outline-none cursor-pointer"
            :class="chatMode === 'descriptive' ? 'border-primary bg-primary/5' : 'border-outline-variant/40 hover:border-primary/40 hover:bg-surface-high'"
          >
            <div class="flex items-center space-x-3">
              <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" :class="chatMode === 'descriptive' ? 'bg-primary/15 text-primary' : 'bg-surface-high text-on-surface-variant'">
                <PenLineIcon class="w-4 h-4" />
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center space-x-2">
                  <span class="text-xs font-bold" :class="chatMode === 'descriptive' ? 'text-primary' : 'text-on-surface'">包含描写</span>
                  <span v-if="chatMode === 'descriptive'" class="text-[8px] px-1.5 py-0.5 rounded-md bg-primary text-white font-bold">当前</span>
                </div>
          ...
          <button
            ...
            class="w-full text-left p-4 rounded-2xl border-2 transition-all focus:outline-none cursor-pointer"
            :class="chatMode === 'director' ? 'border-primary bg-primary/5' : 'border-outline-variant/40 hover:border-primary/40 hover:bg-surface-high'"
          >
            <div class="flex items-center space-x-3">
              <div class="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" :class="chatMode === 'director' ? 'bg-primary/15 text-primary' : 'bg-surface-high text-on-surface-variant'">
                <BookOpenIcon class="w-4 h-4" />
              </div>
              <div class="flex-1 min-w-0">
                <div class="flex items-center space-x-2">
                  <span class="text-xs font-bold" :class="chatMode === 'director' ? 'text-primary' : 'text-on-surface'">导演模式</span>
                  <span v-if="chatMode === 'director'" class="text-[8px] px-1.5 py-0.5 rounded-md bg-primary text-white font-bold">当前</span>
                </div>
...
const chatMode = ref<'descriptive' | 'dialogue' | 'director'>('dialogue') // 当前打开角色的聊天模式（切换角色时从 DB 重新加载）
...
async function setChatMode(mode: 'dialogue' | 'descriptive' | 'director') {
  ...
  const charId = selectedCharacterId.value
  // 1. 立即更新本地 ref（仅影响当前角色的显示，其他角色不受影响）
  chatMode.value = mode
...
async function selectCharacter(charId: string) {
  ...
  if (isGroup) {
    ...
    pendingChatModeLoad = window.api.invoke('get-character-chat-mode', { characterId: charId }).then((modeRes: any) => {
      if (selectedCharacterId.value === charId && modeRes.success && modeRes.mode) {
        chatMode.value = modeRes.mode
      }
    }).catch(() => { /* 保持默认值 */ }).finally(() => {
      pendingChatModeLoad = null
    })
...
onMounted(async () => {
  ...
  window.api.receive('character-chat-mode-changed', (data: { characterId: string; mode: string }) => {
    const currentId = selectedCharacterId.value
    if (currentId && data.characterId === currentId && !isGroupActive.value) {
      chatMode.value = data.mode as 'dialogue' | 'descriptive' | 'director'
    }
  })
  ...
  window.api.receive('receive-message', (msg: any) => {
    ...
    if (msg.role === 'assistant') {
      ...
      // await pendingChatModeLoad 会让 handleAssistantResponse 先完成并清空 pendingDialogueContents，
      // 导致 await 后再检查时已为空，误判非重复。同步检查确保 100% 可靠拦截。
