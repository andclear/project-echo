...
export class DatabaseService {
  ...
  private runDatabaseMigrations(): void {
    ...
    const migrations = [
      ...
      },
      {
        version: 12,
        up: (db: Database.Database) => {
          // 为 Moments 表和 ForumPosts 表新增 is_nsfw 字段，标记内容是否属于 NSFW
          try {
            const pragmaMoments = db.prepare("PRAGMA table_info(Moments)").all() as any[];
            const hasMomentsCol = pragmaMoments.some((c: any) => c.name === 'is_nsfw');
            if (!hasMomentsCol) {
              db.exec("ALTER TABLE Moments ADD COLUMN is_nsfw INTEGER DEFAULT 0;");
            }
          } catch (_) {}

          try {
            const pragmaPosts = db.prepare("PRAGMA table_info(ForumPosts)").all() as any[];
            const hasPostsCol = pragmaPosts.some((c: any) => c.name === 'is_nsfw');
            if (!hasPostsCol) {
              db.exec("ALTER TABLE ForumPosts ADD COLUMN is_nsfw INTEGER DEFAULT 0;");
            }
          } catch (_) {}

          // ==================== 大剧院 (Theater) 跑团系统相关表 ====================
          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterThemes (
              id TEXT PRIMARY KEY,
              name TEXT NOT NULL,
              description TEXT,
              folder_name TEXT NOT NULL,
          ...
          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterSessions (
              id TEXT PRIMARY KEY,
              theme_id TEXT NOT NULL,
              player_character TEXT NOT NULL,
              npc_states TEXT NOT NULL,
              active_effects TEXT,
              inventory TEXT,
              turn_count INTEGER DEFAULT 0,
              updated_at INTEGER NOT NULL,
              FOREIGN KEY (theme_id) REFERENCES TheaterThemes(id)
            );
          `);

          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterMessages (
              id TEXT PRIMARY KEY,
              ...
              metadata TEXT,
              created_at INTEGER NOT NULL,
              FOREIGN KEY (session_id) REFERENCES TheaterSessions(id)
            );
          `);

          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterRelationNodes (
              id TEXT PRIMARY KEY,
              session_id TEXT NOT NULL,
          ...
          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterRelationEdges (
              id TEXT PRIMARY KEY,
              session_id TEXT NOT NULL,
          ...
          try {
            db.exec(`CREATE INDEX IF NOT EXISTS idx_theater_sessions_theme ON TheaterSessions(theme_id);`);
            db.exec(`CREATE INDEX IF NOT EXISTS idx_theater_messages_session ON TheaterMessages(session_id);`);
            db.exec(`CREATE INDEX IF NOT EXISTS idx_theater_rel_nodes_session ON TheaterRelationNodes(session_id);`);
            db.exec(`CREATE INDEX IF NOT EXISTS idx_theater_rel_edges_session ON TheaterRelationEdges(session_id);`);
          } catch (_) {}
        }
      },
      {
        version: 13,
        up: (db: Database.Database) => {
          // 创建大剧院运行状态表 TheaterSessionStates
          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterSessionStates (
          ...
          // 创建大剧院专属向量数据库表 TheaterMessageEmbeddings
          db.exec(`
            CREATE TABLE IF NOT EXISTS TheaterMessageEmbeddings (
              round_id       TEXT NOT NULL,
              session_id     TEXT NOT NULL,
              embedding_json TEXT NOT NULL,
              content_text   TEXT NOT NULL,
          ...
          try {
            db.exec(`CREATE INDEX IF NOT EXISTS idx_theater_embeddings_session ON TheaterMessageEmbeddings(session_id);`);
          } catch (_) {}
        }
      },
      {
        ...
        up: (db: Database.Database) => {
          try {
            db.exec(`ALTER TABLE TheaterSessionStates ADD COLUMN next_options TEXT;`);
          } catch (err: any) {
            console.warn('[Database] 迁移 v14: 尝试添加 next_options 列，可能已存在:', err.message);
          }
        }
      },
      {
        version: 15,
        up: (db: Database.Database) => {
          // 1. 创建粉丝画像表 (仅记录真实角色与用户)
          db.prepare(`
            CREATE TABLE IF NOT EXISTS LiveStreamUserStats (
              char_name TEXT PRIMARY KEY,
              level INTEGER DEFAULT 1,
              exp INTEGER DEFAULT 0,
              total_donated INTEGER DEFAULT 0,
              last_gift_id TEXT,
              last_gift_time INTEGER,
              remarks TEXT
            );
          `).run();

          // 2. 创建直播会话总结表 (has_met_event 强标记奔现)
          db.prepare(`
            CREATE TABLE IF NOT EXISTS LiveStreamSessions (
              id TEXT PRIMARY KEY,
          ...
          // 3. 创建打赏流水日志表 (用于滑动窗口防破产)
          db.prepare(`
            CREATE TABLE IF NOT EXISTS LiveStreamGiftLogs (
              id TEXT PRIMARY KEY,
              session_id TEXT NOT NULL,
      ...
      },
      {
        version: 16,
        up: (db: Database.Database) => {
          try { db.prepare(`ALTER TABLE LiveStreamSessions ADD COLUMN locked_artist TEXT;`).run(); } catch (_) {}
          try { db.prepare(`ALTER TABLE LiveStreamSessions ADD COLUMN locked_artist_name TEXT;`).run(); } catch (_) {}
          try { db.prepare(`ALTER TABLE LiveStreamSessions ADD COLUMN enable_background_rotation INTEGER DEFAULT 0;`).run(); } catch (_) {}
          try {
            db.prepare(`ALTER TABLE LiveStreamUserStats RENAME TO LiveStreamUserStats_old;`).run();
            db.prepare(`
              CREATE TABLE LiveStreamUserStats (
                char_name TEXT NOT NULL,
                ...
                last_gift_time INTEGER,
                remarks TEXT,
                PRIMARY KEY (char_name, host_name)
              );
            `).run();
            db.prepare(`
              INSERT INTO LiveStreamUserStats (char_name, host_name, level, exp, total_donated, last_gift_id, last_gift_time, remarks)
              SELECT char_name, '', level, exp, total_donated, last_gift_id, last_gift_time, remarks
              FROM LiveStreamUserStats_old;
            `).run();
            db.prepare(`DROP TABLE LiveStreamUserStats_old;`).run();
          } catch (err: any) {
            console.warn('[Database] 迁移 v16 (LiveStreamUserStats 联合主键升级) 尝试中，可能已升级:', err.message);
          }
        }
      },
      {
        ...
        up: (db: Database.Database) => {
          ...
          `);
          db.exec(`
            CREATE TABLE IF NOT EXISTS TherapyMessages (
              id TEXT PRIMARY KEY,
              session_id TEXT NOT NULL,
              role TEXT NOT NULL,
              content TEXT NOT NULL,
              timestamp INTEGER NOT NULL,
              inner_thought TEXT DEFAULT NULL,
              token_usage INTEGER DEFAULT 0
            );
          `);
          try {
            db.exec(`CREATE INDEX IF NOT EXISTS idx_therapy_messages_session ON TherapyMessages (session_id);`);
          } catch (_) {}
        }
      },
      {
        ...
        up: (db: Database.Database) => {
          db.exec(`
            CREATE TABLE IF NOT EXISTS ShudongCards (
              id TEXT PRIMARY KEY,
              content TEXT NOT NULL,
          ...
          `);
          db.exec(`
            CREATE TABLE IF NOT EXISTS ShudongComments (
              id TEXT PRIMARY KEY,
  ...
  public saveMoment(moment: {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO Moments (id, character_id, author_name, author_avatar, content, timestamp, likes, liked, is_nsfw)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET
  ...
  public saveForumPost(post: {
    ...
    const stmt = this.db.prepare(`
      INSERT INTO ForumPosts (id, character_id, author_name, author_avatar, title, content, timestamp, views, replies_count, board_id, is_nsfw)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
      ON CONFLICT(id) DO UPDATE SET
