...
6. **对白对话占比与格式绝对铁律**：开篇中出现的所有角色台词和说话内容，必须且只能使用双引号 \`""\` 括起来，严禁使用其它怪异的括弧。**特别声明：生成的整篇小说开篇内容中，角色之间的对话台词对白内容（即用双引号 "" 括起来的说话台词，包括主角与角色的台词互动）在篇幅或字数上必须占据不少于 30% 的绝对比例！严禁通篇只有冗长沉闷的内心独白或环境描写，必须保证高密度、生动有趣的台词碰撞与动作交互，让小说一开篇就充满戏剧张力！**
7. **极简大气质朴文风（物理级限制）**：文风必须**极其简单、直白、干净、朴实。严禁堆砌任何浮夸、晦涩、高大上或极其抽象的意识流形容词，绝不使用矫揉造作的做作词藻与宏大空洞叙事，不要给用户增加任何多余的阅读负担**。请采用高度精炼、画面感强烈的白描动作与物理写实环境速写，用最直白质朴的白话笔触，勾勒最惊心动魄的戏剧张力。
8. **语言**：必须完全使用简体中文进行创作，不带任何 Markdown 格式（严禁使用 \`**加粗**\`、\`### 标题\` 等排版标记），保持干净利落的小说文本状态。
