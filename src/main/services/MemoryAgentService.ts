...
3. 撰写格式规范：
   ...
   - 严禁撰写指令性或提示词式的句子。
   - **严禁在记忆条目中使用相对时间词**（如"明天"、"下周"、"昨天"、"后天"等）！必须将相对时间转换为具体的绝对日期或改为无时间歧义的事实陈述。
   - 所有提取的记忆必须使用【简体中文】。
...
5. **绝对禁止使用“已发生/过去式/正在进行”的日记文体描述未来日程 (Strict Future Planning Tense Rule)**：
   ...
   - 必须且只能使用“计划...”、“准备...”、“打算...”、“希望能...”、“期待...”等符合【尚未发生的未来打算与计划口吻】（例如：“计划今天下午两点给 {{user}} 补课，打算准备一下第三章 of 生物考点，有些期待与对方见面” 或 “打算下山去给为那小子带一串糖葫芦”）。
   - 违反此项规定会导致数字生命的时空记忆概念彻底崩塌与错乱，请务必严守本条红线！
...
[
  ...
  public async consolidateCharacterMemoryFacts(
    ...
    const history = isDialogue ? mergeChatHistory(rawHistory) : rawHistory;
    const historyContext = history.length > 0
      ? history.map((m: any) => `[${m.role === 'user' ? 'User' : 'Character'}]: ${cleanContentForLLM(m.content)}`).join('\n')
      : '（暂无聊天历史）';
