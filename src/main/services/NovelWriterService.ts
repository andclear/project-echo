...
【关于章节标题】⚠️ 章节标题拟定硬约束：由你自行拟定具体、独立、富有网文文采的小说章节标题（如《月下风铃》等），6~15字为宜。绝对严禁直接使用形如“第X章”、“第9章”、“第九章”或类似的章回纯序号作为标题名称！标题中绝对不能包含任何“第X章”或“第9章”等数字序号。`
      ...
      for (let i = 0; i < splitParts.length; i++) {
        ...
        const rawPartTitle = splitParts.length === 1
          ? rawTitle
          : (part.title && part.title.trim() ? part.title.trim() : `${rawTitle}（${i + 1}）`)
        const partTitle = this.getUniqueChapterTitle(characterId, rawPartTitle, newChapterIndex + i, db)
...
4. 【⚠️ 章节标题拟定硬约束】：每一章都必须拟定具体、独立、富有网文文采的小说章节名称（如《月下风铃》、《图书馆里的秘密》等）。绝对严禁直接使用形如“第X章”、“第9章”、“第九章”或类似的章回纯序号作为标题名称！

返回格式要求：
- 若需要拆分，请仅返回一个 JSON 数组，每个元素包含 "title"（本章参考标题）和 "content" ...
