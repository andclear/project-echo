...
创作要点：①用场景和动作包裹台词，绝不逐条翻译 ②让读者始终清楚谁在说话 ③用行动展示情绪而非直接陈述
不要写任何开场白或多余解释，正文完成后另起一行输出标题，格式固定为：
### TITLE: {章节标题}
...
【关于章节标题】由你自行拟定，简洁有力、贴合本章情感基调，6~15字为宜，无需使用"第X章"字样。`
  ...
  public async rewriteChapter(chapterId: string): Promise<void> {
    ...
    try {
      ...
      const userPrompt = `【待改编的对话原材料（按时间顺序）】\n${formattedDialogue}\n\n请立即以小说叙事手法将以上对话改编成一个完整的小说章节。\n创作要点：①用场景和动作包裹每一句台词，绝不逐条翻译 ②让读者始终清楚谁在说话 ③用行动展示情绪而非直接陈述\n不要写任何开场白或多余解释，正文完成后另起一行输出标题，格式固定为：\n### TITLE: {章节标题}`
...
禁止做的：新增原文中完全没有提及的情节转折、改变对话含义`
      ...
      case 'free':
        return `【改编尺度：自由创作】
核心原则：对话是情感锚点和事件骨架，具体怎么呈现完全由你决定。
...
【最重要的前提：对话是素材，不是剧本】
你收到的聊天记录只是创作的原材料。你的任务是把它们转化成有场景、有动作、有氛围的小说正文。
绝对禁止：将对话逐条翻译成小说台词——那只是换格式，不是创作。
每一句对话在进入正文前，都要思考：这句话发生时，人物在哪里、在做什么、脸上是什么表情？用这些细节包裹台词，才是小说。
