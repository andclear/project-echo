...
## 【最后消息焦点响应铁律 — 绝对最高优先级，任何情况不得违反】
- **本轮对话的核心主题与台词，必须 100% 严格地以用户最新发来的发言（即 \`{role: 'user'}\` 消息最末梢的内容）为核心进行展开！**
- 如果用户突然转移了对话话题（例如上一句聊游戏，本轮聊茶叶，或上一句在谈感情，本轮正经谈论工作等），你**必须极度敏锐地感知到话题和对话氛围的瞬间切换**。
- 你的回复（无论是口语台词还是肢体动作描写）**必须立刻且优先正面回应用户本轮的新发言，神态动作和语气必须无缝顺应新话题，绝对禁止装作没看见、漏读新消息或强行执拗地粘着在之前的旧话题中！**
...
如果你由于余额不足或性格人设不想给钱，你【绝对绝对禁止】在文字台词或动作描写中瞎编、声称、假装说“给你转账了”、“发过去了”、“转了XX万”！没钱就必须符合人设地进行傲娇拒绝或哭穷，绝对不能做无实物的假转账嘴炮！`;
    ...
    // 7. 自动加载线下约会与奔现记录 meet_records.json
    const meetRecordsPath = path.join(path.dirname(soulPath), 'meet_records.json');
    if (fs.existsSync(meetRecordsPath)) {
      try {
        const records = JSON.parse(fs.readFileSync(meetRecordsPath, 'utf-8'));
        if (Array.isArray(records) && records.length > 0) {
          const recentMeets = records.slice(-3);
          const meetLines = recentMeets.map(r => {
            const partnerDesc = r.partnerType === 'user' ? '用户本人' : 'VIP观众';
            const meetTypeDesc = r.type === 'meet' ? '线下奔现约会' : '线下亲密约会';
            return `- ${r.date}，与 ${r.partnerName} (${partnerDesc}) 达成了${meetTypeDesc}的约定并确认成功。`;
          }).join('\n');
          dynamicContext += `## 线下约会与奔现事件 (Recent Meet & Date Events)\n${meetLines}\n\n`;
        }
      } catch (_) {}
    }
  ...
  public static assembleLiveEnvInfo(date: Date = new Date()): string {
    ...
    const monthName = partMap.month || monthNames[date.getMonth()];
    const day = partMap.day || String(date.getDate());
    const hours = (partMap.hour || String(date.getHours())).padStart(2, '0');
    const dayNameEN = partMap.weekday || dayNamesEN[date.getDay()];
    const hourNum = partMap.hour ? parseInt(partMap.hour, 10) : date.getHours();

    const weekdayMapCN: Record<string, string> = {
      'Sunday': '星期日', 'Monday': '星期一', 'Tuesday': '星期二',
      'Wednesday': '星期三', 'Thursday': '星期四', 'Friday': '星期五', 'Saturday': '星期六'
    };
    const dayNameCN = weekdayMapCN[dayNameEN] || '星期一';

    // 采用 toLocaleDateString 获取数字月份，确保符合 'zh-CN' 时区对应的月份数字
    let monthNumStr = '1';
    try {
      monthNumStr = date.toLocaleDateString('zh-CN', { timeZone: 'Asia/Shanghai', month: 'numeric' }).replace('月', '');
    } catch (_) {
      monthNumStr = String(date.getMonth() + 1);
    }
    ...
    const dateStrCN = `${year}年${monthNumStr}月${day}日 ${dayNameCN} 【时段：${timeOfDay}】 ${hours}时`;
