...
### 第四步：执行客户端打包
运行您的打包构建命令（项目使用 `electron-vite` 与 `electron-builder` 进行打包）：
```bash
npm run build
```
打包成功后，将在 `dist/` 或 `release/` 目录中生成对应系统的分发包：
* **Windows**：`.exe` 安装程序（例如 `Echo-Setup-1.1.0.exe`）
* **macOS**：`.dmg` 镜像（例如 `Echo-1.1.0-arm64.dmg` 或 `Echo-1.1.0-x64.dmg`）
* **Linux**：`.AppImage` 包（例如 `Echo-1.1.0.AppImage`）
