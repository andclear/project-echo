...
-->`, 'utf-8');
  ...
  describe('4. assembleLiveEnvInfo 时区独立感知单元测试', () => {
    it('无论当前 Node 进程时区为何，均能正确格式化并输出上海时区的北京时间', () => {
      // 设定一个固定的绝对时间戳（对应北京时间 2026-06-22 12:30:00，此时 UTC 为 04:30:00）
      const testDate = new Date(Date.UTC(2026, 5, 22, 4, 30, 0));
      const envInfo = ContextAssembler.assembleLiveEnvInfo(testDate);
      
      expect(envInfo).toContain('2026年6月22日');
      expect(envInfo).toContain('12时');
      expect(envInfo).toContain('【时段：中午】');
      expect(envInfo).toContain('Monday, June 22, 2026 12:00');
    });

    it('无论当前 Node 进程时区为何，凌晨四点（北京时间）均能正确解析', () => {
      // 北京时间 2026-06-22 04:15:00 时，UTC 时间是 前一天（21日） 20:15:00
      const testDate = new Date(Date.UTC(2026, 5, 21, 20, 15, 0));
      const envInfo = ContextAssembler.assembleLiveEnvInfo(testDate);

      expect(envInfo).toContain('2026年6月22日');
      expect(envInfo).toContain('04时');
      expect(envInfo).toContain('【时段：深夜】');
      expect(envInfo).toContain('Monday, June 22, 2026 04:00');
    });
  });
});
